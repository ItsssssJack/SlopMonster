...
if [ "${1:-}" = "-" ] || [ $# -eq 0 ]; then DRAFT="$(cat)"; else DRAFT="$(cat "$1")"; fi
...
if [ -n "$CODEX" ] && [ "${DESLOP_WRITER:-claude}" != "gpt" ]; then
  ...
  echo "$RAW" | awk '$0=="codex"{buf="";on=1;next} $0=="tokens used"{on=0;next} on{buf=buf $0 "\n"} END{printf "%s", buf}' \
    | { read -r first; if [ -n "$first" ]; then echo "$first"; cat; else echo "$RAW"; fi; }
elif [ -n "$CLAUDE" ]; then
  # GPT wrote the draft (DESLOP_WRITER=gpt, or no codex CLI) -> Claude cleanses it.
  run_bounded "$CLAUDE" -p "$FULL"
else
  echo "deslop: no codex or claude CLI found. Paste the block below into a" >&2
  echo "different model family than the one that wrote your draft:" >&2
  echo >&2
  printf '%s\n' "$FULL"
  exit 127
fi
